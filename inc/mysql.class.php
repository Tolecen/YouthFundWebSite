...
class proverb_mysql {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $halt = TRUE, $dbcharset2 = '') {
		...
		if(!$this->link = @$func($dbhost, $dbuser, $dbpw, 1)) {
			...
			if($this->version() > '4.1') {
				global $proverbcharset, $proverbdbcharset;
				$dbcharset = $dbcharset2 ? $dbcharset2 : $proverbdbcharset;
				$dbcharset = !$dbcharset && in_array(strtolower($proverbcharset), array('utf-8', 'big5', 'utf8','gbk')) ? str_replace('-', '', $proverbcharset) : $dbcharset;
	...
	function halt($message = '', $sql = '') {
		$timestamp=time();
		if($message) {
		$errmsg = "<b>PROVERB info</b>: $message<br/>";
		}
		$errmsg .= "<b>Time</b>: ".gmdate("Y-n-j g:ia", $timestamp)."<br/>";
		$errmsg .= "<b>Script</b>: ".$_SERVER['PHP_SELF']."<br/>";
		if($sql) {
			$errmsg .= "<b>SQL</b>: ".htmlspecialchars(str_replace($GLOBALS['proverbtable_prefix'],'',$sql))."<br/>";
		}
